...
describe("test CheckPermissions", () => {
  ...
  it("currentAuthority is undefined , return error", () => {
    expect(
      checkPermissions("communityManage", null, target, error)
    ).toEqual("error");
  });
  it("Wrong string permission authentication", () => {
    expect(
      checkPermissions("communityManage", "user", target, error)
    ).toEqual("error");
  });
  it("Correct Array permission authentication", () => {
    expect(
      checkPermissions(["user", "communityManage"], "user", target, error)
    ).toEqual("ok");
  });
  it("Wrong Array permission authentication,currentAuthority error", () => {
    expect(
      checkPermissions(
        ["user", "communityManage"],
        "user,communityManage",
        target,
        error
      )
  ...
  it("Wrong Array permission authentication", () => {
    expect(
      checkPermissions(["user", "communityManage"], "guest", target, error)
    ).toEqual("error");
  });
  ...
  it("authority is string, currentAuthority is array, return ok", () => {
    expect(
      checkPermissions("user", ["user", "communityManage"], target, error)
    ).toEqual("ok");
  });
