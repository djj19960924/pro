      ...
      callback: res => {
        ...
        if(res.code == 200){
          ...
          if(t == 'update'){
            ...
            router.push(rc + '/news/newsList/list');
          }
        }
      },
    });
      ...
      <Card bordered={false}>
        <Form onSubmit={this.handleSubmit} style={{ marginTop: 8 }}>
          ...
          <FormItem {...formItemLayout} label='标签'>
            {getFieldDecorator('hot_status', {
              rules: [{ message: '请选择'}],
            })(
              <Radio.Group disabled={isDisabled}>
                <Radio value="1">荐</Radio>
                <Radio value="2">热</Radio>
                <Radio value="-1">无</Radio>
              </Radio.Group>
            )}
          </FormItem>
          <FormItem {...formItemLayout} label='责任编辑'>
          ...
          <FormItem {...formItemLayout} label='摄像者'>
            {getFieldDecorator('photo_user', {
              rules: [{ required: true, message: '请输入(小于20个字符)', whitespace: true, max: 20 }],
            })(<Input placeholder='请输入' disabled={isDisabled}/>)}
          </FormItem>
          ...
          <FormItem {...submitFormLayout} style={{ marginTop: 32 }}>
            <Button style={{ marginRight: 8 }}>
              <Link to={`${rc}/news/newsList/list`}>返回</Link>
            </Button>
            {
