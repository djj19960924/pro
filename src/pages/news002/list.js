...
import { rc,fileUrl,news002Id } from '@/global';
...
@connect(({ news002, loading }) => ({
  news002,
  loading: loading.models.news002,
}))
@Form.create()
class news extends PureComponent {
  ...
  columns = [
    ...
    },
    {
      ...
      render: (text, record) => (
        <Fragment>
          <Link onClick={() => this.setNewsInfo(record)} to={`${rc}/ydzx/add/index?id=${record.id}&disabled=t`}>详情</Link>
  ...
  menu = record => {
    return (
      <Menu onClick={e => this.edit(e, record)}>
      {
        ...
        <Menu.Item key="1">
          <Link to={`${rc}/ydzx/add/index?id=${record.id}`}>修改</Link>
        </Menu.Item>
