...
class tice extends PureComponent {
  ...
  columns = [
    ...
    },
    {
      ...
      render: (text, record) => (
        <Fragment>
          <a onClick={() => this.toReport(record)}>指导</a>
        </Fragment>
      ),
    },
    {
      ...
      render: (text, record) => (
        <Fragment>
          <Link to={`${rc}/guide/guide/chufang?id=${record.reportcode}`}>指导</Link>
        </Fragment>
      ),
    },
  ];
  ...
  toReport = record => {
    ...
    router.push(`${rc}/guide/guide/report`);
  }
      ...
      return (
        <div>
          ...
          <div className={styles.moreRecord}>
            <Link
              to={`${rc}/guide/guide/moreData?openId=${record.openid}`}
