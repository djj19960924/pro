    ...
    return (
      ...
      <Card bordered={false}>
        <Form onSubmit={this.handleSubmit} style={{ marginTop: 8 }}>
          ...
          <FormItem {...formItemLayout} label="发送的社团">
            {getFieldDecorator('matchEventId',{
              rules: [{ required: true, message: '请选择想要发送的社团'}],
            })(
              <Select placeholder="请选择" style={{ width: '100%' }}>
                {
                  this.state.clubList.map(ele => {
                    return <Option key={ele.matchEventId}>{ele.eventName}</Option>
                  })
                }
              </Select>
            )}
