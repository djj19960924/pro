    ...
    return (
      ...
      <Card bordered={false}>
        <Form onSubmit={this.handleSubmit} style={{ marginTop: 8 }}>
          ...
          <FormItem {...formItemLayout} label="发送的赛事">
            {getFieldDecorator('competitionId',{
              rules: [{ required: true, message: '请选择想要发送的赛事'}],
            })(
              <Select placeholder="请选择" style={{ width: '100%' }}>
                {
                  this.state.gameList.map(ele => {
                    return <Option key={ele.competitionId}>{ele.competitionName}</Option>
                  })
                }
              </Select>
            )}
